-- JFD_HungaryFunctions
-- Author: JFD
-- DateCreated: 1/17/2014 12:14:14 AM
--==========================================================================================================================
-- Includes
--==========================================================================================================================
include("JFD_HungaryDTPSupport.lua")
--==========================================================================================================================
-- UTILITY FUNCTIONS
--==========================================================================================================================
...
--------------------------------------------------------------------------------------------------------------------------
-- JFD_GetNumDefensiveBuildingsInCity
--------------------------------------------------------------------------------------------------------------------------
function JFD_GetNumDefensiveBuildingsInCity(city)
...
function JFD_HungarianVegvar(playerID)
	...
	if (player:IsAlive() and player:GetCivilizationType() == civilisationID) then
		...
		for city in player:Cities() do
			if (city:IsHasBuilding(buildingVegvarID) and religionID > -1 and city:GetReligiousMajority() == religionID) then
				city:SetNumRealBuilding(buildingVegvarFaithID, JFD_GetNumDefensiveBuildingsInCity(city))
	...
	if player:IsAlive() and not player:IsMinorCiv() and not player:IsBarbarian() then
		for otherPlayerID = 0, GameDefines.MAX_CIV_PLAYERS-1 do
			if Players[otherPlayerID]:GetCivilizationType() == civilisationID and player:IsAlive() then
				if Teams[player:GetTeam()]:IsAtWar(otherPlayerID) then
					for city in player:Cities() do
