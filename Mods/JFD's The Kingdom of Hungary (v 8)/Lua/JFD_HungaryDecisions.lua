...
local Decisions_HungaryArchdiocese = {}
	...
	function(player)
		...
		ConvertPlayerReligion(player, religionTypeID, 100)
		
		if player:IsHuman() then
			LuaEvents.JFD_ShowChooseArchdiocesePopup()
		else
			player:GetCapitalCity():SetNumRealBuilding(buildingCathedralID, 1)
			JFD_SendWorldEvent(player:GetID(), Locale.ConvertTextKey("TXT_KEY_WORLD_EVENT_JFD_HUNGARY_ARCHDIOCESE", player:GetCivilizationDescription())) 
		end
